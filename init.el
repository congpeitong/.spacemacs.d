...
;; 在这里可以声明一些layer 以及删除，增加一些包，还可以调整spacemacs加载时的一些行为
...
This function should only modify configuration layer settings."
  (setq-default
   ...
   ;; spacemacs安装完整版(spacemacs)/精简版(spacemacs-base)
   ...
   ;; 如果你配置了自己的layer,却把他放在了 .emacs.d以外的文件夹中，为了让spacemacs可以找到这个layer，就需要配置路径值为一个目录的路径，结尾要带上 "/"
   dotspacemacs-configuration-layer-path '()

   ;; List of configuration layers to load.
   ;; 声明layer的地方，将会在下次启动spacemacs时自动安装，也可以进行一些layer的配置
...
;; spacemacs绝大部分的配置都位于此，可以在此修改配置中可选的选项，但是绝不能将自己的用户配置代码添加在这里
...
;; 这里的内容会在Emacs启动前开始加载，一般在这里设置你需要使用的elpa源，应该尽量把用户配置放在dotspacemcs/user-config中
...
If you are unsure, try setting them in `dotspacemacs/user-config' first."
  ...
  ;; 选择的不是emacs编辑模式，可能会有一条evil-want-keybinding警告，可去除警告
...
;; 在这里可以添加用户配置代码，自己定义的大部分配置都在这里完成
